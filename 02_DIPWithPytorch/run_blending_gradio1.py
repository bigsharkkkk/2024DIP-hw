...
def add_point(img_original, polygon_state, evt: gradio.SelectData):
    """
    ...
    Args:
        ...
        evt (gradio.SelectData): The click event data.
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    """masked_grad_x1 = foreground_grad_x * foreground_mask
    masked_grad_y1 = foreground_grad_y * foreground_mask

    masked_grad_x1 = masked_grad_x1[masked_grad_x1 != 0]
    masked_grad_y1 = masked_grad_y1[masked_grad_y1 != 0]

    masked_grad_x2 = blended_grad_x * background_mask
    masked_grad_y2 = blended_grad_y * background_mask
    masked_grad_x2 = masked_grad_x2[masked_grad_x2 != 0]
    masked_grad_y2 = masked_grad_y2[masked_grad_y2 != 0]"""
...
# Gradio Interface
with gradio.Blocks(title="Poisson Image Blending", css="""
    body {
...
""") as demo:
    # Initialize states
    polygon_state = gradio.State(initialize_polygon())
    background_image_original = gradio.State(value=None)

    # Title and description
    gradio.Markdown("<h1 style='text-align: center;'>Poisson Image Blending</h1>")
    gradio.Markdown("<p style='text-align: center; font-size: 1.2em;'>Blend a selected area from a foreground image onto a background image with adjustable positions.</p>")

    with gradio.Row():
        with gradio.Column():
            gradio.Markdown("### Foreground Image")
            foreground_image_original = gradio.Image(
            ...
            gradio.Markdown("### Foreground Image with Polygon")
            foreground_image_with_polygon = gradio.Image(
                label="", type="pil", interactive=True, height=300
            )
            gradio.Markdown(
            ...
            close_polygon_button = gradio.Button("Close Polygon")
        with gradio.Column():
            gradio.Markdown("### Background Image")
            background_image = gradio.Image(
                label="", type="pil", interactive=True, height=300
            )
            gradio.Markdown("<p style='font-size: 0.9em;'>Upload the background image where the polygon will be placed.</p>")

    with gradio.Row():
        with gradio.Column():
            gradio.Markdown("### Background Image with Polygon Overlay")
            background_image_with_polygon = gradio.Image(
                label="", type="pil", height=500
            )
            gradio.Markdown("<p style='font-size: 0.9em;'>Adjust the position of the polygon using the sliders below.</p>")
        with gradio.Column():
            gradio.Markdown("### Blended Image")
    ...
    with gradio.Row():
        ...
        blend_button = gradio.Button("Blend Images")
